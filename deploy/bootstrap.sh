...
API_IMAGE=ghcr.io/levisbakalinsky/sailing-plans-api:latest
WEB_IMAGE=ghcr.io/levisbakalinsky/sailing-plans-web:latest
EOF
  chmod 600 .env
  echo "Wrote $APP_DIR/.env — edit secrets before first deploy."
fi

echo "Bootstrap complete. Copy deploy/docker-compose.yml and deploy/Caddyfile here, then run GitHub Actions deploy."
